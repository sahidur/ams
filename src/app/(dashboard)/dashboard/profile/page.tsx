...
export default function ProfilePage() {
  ...
  // Fetch login logs when timeline tab is active and filter is login_information or all
  useEffect(() => {
    const fetchLoginLogs = async () => {
      if (!session?.user?.id || activeTab !== "timeline") return;
      if (activityFilter !== "all" && activityFilter !== "LOGIN_INFORMATION") return;
      
      setIsLoadingLoginLogs(true);
      try {
        const res = await fetch(`/api/users/${session.user.id}/login-logs?limit=50`);
        const data = await res.json();
          ...
          <div className="p-6">
            ...
            {activeTab === "timeline" && (
              <div className="space-y-6">
                ...
                {isLoadingActivity || isLoadingLoginLogs ? (
